...
while True:
    ...
    if features["交差点 右折"] or (
            black_amount_upper < 10 and black_amount_bottom > 75 and black_center_position_weighted_bottom > 0):
        control = 90  # 右に曲がりたい
    if features["交差点 左折"] or (
            black_amount_upper < 10 and black_amount_bottom > 75 and black_center_position_weighted_bottom < 0):
        control = -90  # 左に曲がりたい
    if control is not np.nan:
        client.sendto(str(control).encode('utf-8'), (HOST, PORT))
